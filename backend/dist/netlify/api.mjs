...
var rateLimit = (options) => {
  return async (c, next) => {
    ...
    const key = options.keyBuilder ? options.keyBuilder(c) : `rl:${clientIp}:${path2}`;
    const now = Date.now();
    try {
...
var OneDriveProvider = class {
  ...
  normalizePath(path2) {
    let p = path2.replace(/\\/g, "/").replace(/\/+/g, "/").trim();
    if (!p.startsWith("/")) p = "/" + p;
...
var BaiduNetdiskProvider = class {
  ...
  async uploadBackup(filename, content) {
    const token = await this.getAccessToken();
    const path2 = `${this.saveDir}/${filename}`;
    ...
    const uploadUrl = `https://d.pcs.baidu.com/rest/2.0/pcs/superfile2?method=upload&type=tmpfile&path=${encodeURIComponent(path2)}&uploadid=${uploadid}&partseq=0&access_token=${token}`;
    const formData = new FormData();
  ...
  async downloadBackup(filename) {
    const token = await this.getAccessToken();
    const path2 = `${this.saveDir}/${filename}`;
    const metaUrl = `https://pan.baidu.com/rest/2.0/xpan/multimedia?method=filemetas&path=${encodeURIComponent(JSON.stringify([path2]))}&dlink=1&access_token=${token}`;
...
var DropboxProvider = class {
  ...
  async listBackups() {
    const token = await this.getAccessToken();
    const path2 = this.saveDir === "/" || this.saveDir === "" ? "" : this.saveDir.startsWith("/") ? this.saveDir : "/" + this.saveDir;
  ...
  async uploadBackup(filename, data) {
    const token = await this.getAccessToken();
    const path2 = this.saveDir === "/" || this.saveDir === "" ? "" : this.saveDir.startsWith("/") ? this.saveDir : "/" + this.saveDir;
    const fullPath = `${path2}/${filename}`.replace(/\/+/g, "/");
    const args = {
      path: fullPath,
      mode: "overwrite",
      autorename: false,
...
var GithubProvider = class {
  ...
  getApiUrl(path2) {
    return `https://api.github.com/repos/${this.owner}/${this.repo}${path2}`;
  }
  ...
  async uploadBackup(filename, content) {
    const path2 = `/contents/${this.saveDir ? this.saveDir + "/" : ""}${filename}`;
    ...
    let sha;
    try {
      const existing = await this.request(path2);
    ...
    await this.request(path2, { method: "PUT", body });
  }
  async downloadBackup(filename) {
    const path2 = `/contents/${this.saveDir ? this.saveDir + "/" : ""}${filename}`;
    const data = await this.request(path2);
  ...
  async deleteBackup(filename) {
    ...
    const body = JSON.stringify(bodyObj);
    await this.request(path2, { method: "DELETE", body });
  }
};
...
wcProxy.all("/rpc/*", (c) => {
  if (c.env.OAUTH_WALLETCONNECT_SELF_PROXY !== "true") throw new AppError("wc_proxy_disabled", 403);
  const path2 = c.req.path.substring(c.req.path.indexOf("/rpc"));
  const targetPath = path2.replace("/rpc", "/v1");
  return proxyRequest("rpc.walletconnect.com", targetPath, c);
});
wcProxy.all("/verify/*", (c) => {
  const path2 = c.req.path.substring(c.req.path.indexOf("/verify"));
  return proxyRequest("verify.walletconnect.com", path2.replace("/verify", ""), c);
});
wcProxy.all("/explorer/*", (c) => {
  const path2 = c.req.path.substring(c.req.path.indexOf("/explorer"));
  return proxyRequest("explorer-api.walletconnect.com", path2.replace("/explorer", ""), c);
});
...
app.use("/api/*", async (c, next) => {
  const path2 = c.req.path;
  if (path2.startsWith("/api/health") || path2 === "/api/oauth/logout") {
    await next();
    return;
  }
...
app.get("*", async (c) => {
  if (c.env && c.env.ASSETS) {
    ...
    const isStaticAsset = path2.startsWith("/assets/") || path2 === "/sw.js" || /\.(js|mjs|css|png|jpg|jpeg|gif|svg|ico|webmanifest|wasm|json|woff2?|ttf|map)$/i.test(path2);
    if (isStaticAsset && contentType.includes("text/html")) {
      logger.warn(`[Static] Prevented SPA fallback for missing asset: ${path2}`);
  ...
  static async create() {
    ...
    switch (engine2) {
      ...
      case "mysql": {
        ...
        const db = drizzleMysql(executor.pool, { schema: mysql_exports, mode: "default" });
        return { executor, db, schema: mysql_exports };
      }
      case "postgres":
      case "postgresql": {
        ...
        const db = drizzlePg(executor.pool, { schema: pg_exports });
      ...
      default: {
        ...
        const db = drizzleSqlite(sqlite, { schema: sqlite_exports });
...
async function applyMigrationStatements(db, engine2, rawSql, label) {
...
async function ensureActiveShareKeyRepair(db, engine2, currentVersion) {
  ...
  await applyMigrationStatements(db, engine2, engineSql, "active-share-key repair");
}
async function migrateDatabase(db) {
  ...
  if (pending.length === 0) {
    await ensureActiveShareKeyRepair(db, engine2, currentVersion);
    return;
  }
  ...
  for (const m of pending) {
    ...
    try {
      ...
      await applyMigrationStatements(db, engine2, engineSql, `v${m.version}`);
...
var handler = async (event, context) => {
  try {
    if (!cachedDb) {
      console.log("\u{1F4E1} [DB] Initializing new connection pool...");
      const { db, executor } = await DbFactory.create();
      cachedDb = { db, executor };
      if (db && typeof db.on === "function") {
        db.on("error", (err) => {
          console.error("\u{1F4E1} [DB Pool] Background Error:", err.message);
          cachedDb = null;
        });
      }
      try {
        ...
        console.error("\u{1F5C4}\uFE0F [DB Migrate] Failed:", err.message);
        cachedDb = null;
        return {
          statusCode: 503,
    ...
    let path2 = event.headers["x-nf-original-path"] || event.path || "/";
    if (path2.startsWith("/.netlify/functions/api")) {
      path2 = path2.replace("/.netlify/functions/api", "/api");
    }
    const query = event.queryStringParameters ? "?" + new URLSearchParams(event.queryStringParameters).toString() : "";
    const url = `${protocol}://${host}${path2}${query}`;
    const request = new Request(url, {
      method: event.httpMethod || "GET",
      headers: event.headers,
      body: event.body ? event.isBase64Encoded ? Buffer.from(event.body, "base64") : event.body : void 0
    });
    const res = await app_default.fetch(request, {
      ...process.env,
      ...context,
      DB: cachedDb?.db
    });
    ...
    if (contentType.includes("application/json")) {
      const text3 = await res.text();
      try {
        body = JSON.stringify(JSON.parse(text3));
      } catch {
        body = text3;
      }
    } else {
    ...
    console.error("\u{1F6D1} [Architect] Runtime Crash:", err);
