...
__export(mysql_exports, {
  authPasskeys: () => authPasskeys2,
  authSessions: () => authSessions2,
  backupEmailHistory: () => backupEmailHistory2,
  backupProviders: () => backupProviders2,
...
var pg_exports = {};
__export(pg_exports, {
  authPasskeys: () => authPasskeys3,
  authSessions: () => authSessions3,
  backupEmailHistory: () => backupEmailHistory3,
  backupProviders: () => backupProviders3,
...
var rateLimit = (options) => {
  return async (c, next) => {
    ...
    const key = options.keyBuilder ? options.keyBuilder(c) : `rl:${clientIp}:${path4}`;
    const now = Date.now();
    try {
      if (typeof db2.prepare === "function") {
        const record = await db2.prepare(
        ...
        const result = await db2.select().from(rateLimits4).where(eq2(rateLimits4.key, key)).limit(1);
        ...
        if (record) {
          ...
          if (now - (record.lastAttempt || 0) > options.windowMs) {
            await db2.update(rateLimits4).set({ attempts: 1, lastAttempt: now, expiresAt: null }).where(eq2(rateLimits4.key, key));
          } else {
            const newAttempts = (record.attempts || 0) + 1;
            if (newAttempts > options.max) {
              const expiresAt = now + options.windowMs;
              await db2.update(rateLimits4).set({ attempts: newAttempts, lastAttempt: now, expiresAt }).where(eq2(rateLimits4.key, key));
              throw new AppError("too_many_requests", 429);
            } else {
              await db2.update(rateLimits4).set({ attempts: newAttempts, lastAttempt: now }).where(eq2(rateLimits4.key, key));
            }
          }
        } else {
          await db2.insert(rateLimits4).values({ key, attempts: 1, lastAttempt: now });
        }
      }
    } catch (e) {
...
var resetRateLimit = async (c, key) => {
  ...
  try {
    if (typeof db2.prepare === "function") {
      await db2.prepare("DELETE FROM rate_limits WHERE key = ?").bind(key).run();
    } else {
      await db2.delete(rateLimits4).where(eq2(rateLimits4.key, key));
    }
  } catch (e) {
...
var TelegramProvider2 = class {
  ...
  db;
  providerId;
  constructor(config, db2, providerId) {
...
var OneDriveProvider = class {
  ...
  normalizePath(path4) {
    let p = path4.replace(/\\/g, "/").replace(/\/+/g, "/").trim();
    if (!p.startsWith("/")) p = "/" + p;
...
var BaiduNetdiskProvider = class {
  ...
  async uploadBackup(filename, content) {
    const token = await this.getAccessToken();
    const path4 = `${this.saveDir}/${filename}`;
    ...
    const uploadUrl = `https://d.pcs.baidu.com/rest/2.0/pcs/superfile2?method=upload&type=tmpfile&path=${encodeURIComponent(path4)}&uploadid=${uploadid}&partseq=0&access_token=${token}`;
    const formData = new FormData();
  ...
  async downloadBackup(filename) {
    const token = await this.getAccessToken();
    const path4 = `${this.saveDir}/${filename}`;
    const metaUrl = `https://pan.baidu.com/rest/2.0/xpan/multimedia?method=filemetas&path=${encodeURIComponent(JSON.stringify([path4]))}&dlink=1&access_token=${token}`;
...
var DropboxProvider = class {
  ...
  async listBackups() {
    const token = await this.getAccessToken();
    const path4 = this.saveDir === "/" || this.saveDir === "" ? "" : this.saveDir.startsWith("/") ? this.saveDir : "/" + this.saveDir;
  ...
  async uploadBackup(filename, data) {
    const token = await this.getAccessToken();
    const path4 = this.saveDir === "/" || this.saveDir === "" ? "" : this.saveDir.startsWith("/") ? this.saveDir : "/" + this.saveDir;
    const fullPath = `${path4}/${filename}`.replace(/\/+/g, "/");
    const args = {
      path: fullPath,
      mode: "overwrite",
      autorename: false,
...
var EmailProvider = class {
  ...
  providerId;
  lang;
  constructor(config, db2, providerId, lang = "en-US") {
...
var GithubProvider = class {
  ...
  getApiUrl(path4) {
    return `https://api.github.com/repos/${this.owner}/${this.repo}${path4}`;
  }
  async request(path4, options = {}) {
    const url = this.getApiUrl(path4);
  ...
  async uploadBackup(filename, content) {
    const path4 = `/contents/${this.saveDir ? this.saveDir + "/" : ""}${filename}`;
    ...
    let sha;
    try {
      const existing = await this.request(path4);
    ...
    await this.request(path4, { method: "PUT", body });
  }
  async downloadBackup(filename) {
    const path4 = `/contents/${this.saveDir ? this.saveDir + "/" : ""}${filename}`;
  ...
  async deleteBackup(filename) {
    ...
    const body = JSON.stringify(bodyObj);
    await this.request(path4, { method: "DELETE", body });
  }
};
...
function createShareService(env, db2 = env.DB) {
  const vaultRepository = new VaultRepository(db2);
  const shareRepository = new ShareRepository(db2);
...
wcProxy.all("/rpc/*", (c) => {
  if (c.env.OAUTH_WALLETCONNECT_SELF_PROXY !== "true") throw new AppError("wc_proxy_disabled", 403);
  const path4 = c.req.path.substring(c.req.path.indexOf("/rpc"));
  const targetPath = path4.replace("/rpc", "/v1");
  return proxyRequest("rpc.walletconnect.com", targetPath, c);
});
wcProxy.all("/verify/*", (c) => {
  const path4 = c.req.path.substring(c.req.path.indexOf("/verify"));
  return proxyRequest("verify.walletconnect.com", path4.replace("/verify", ""), c);
});
wcProxy.all("/explorer/*", (c) => {
  const path4 = c.req.path.substring(c.req.path.indexOf("/explorer"));
  return proxyRequest("explorer-api.walletconnect.com", path4.replace("/explorer", ""), c);
});
...
app.use("/api/*", async (c, next) => {
  const path4 = c.req.path;
  if (path4.startsWith("/api/health") || path4 === "/api/oauth/logout") {
    await next();
    return;
  }
...
app.get("*", async (c) => {
  if (c.env && c.env.ASSETS) {
    ...
    const isStaticAsset = path4.startsWith("/assets/") || path4 === "/sw.js" || /\.(js|mjs|css|png|jpg|jpeg|gif|svg|ico|webmanifest|wasm|json|woff2?|ttf|map)$/i.test(path4);
    if (isStaticAsset && contentType.includes("text/html")) {
      logger.warn(`[Static] Prevented SPA fallback for missing asset: ${path4}`);
  ...
  static async create() {
    ...
    switch (engine2) {
      ...
      case "mysql": {
        ...
        const executor2 = new MySqlExecutor(config);
        const db2 = drizzleMysql(executor2.pool, { schema: mysql_exports, mode: "default" });
        return { executor: executor2, db: db2, schema: mysql_exports };
      }
      case "postgres":
      case "postgresql": {
        ...
        const executor2 = new PgExecutor(config);
        const db2 = drizzlePg(executor2.pool, { schema: pg_exports });
        return { executor: executor2, db: db2, schema: pg_exports };
      }
      case "sqlite":
      default: {
        const rawPath = (process.env.SQLITE_DB_PATH || "./data/nodeauth.db").trim();
        const dbFile = path.isAbsolute(rawPath) ? rawPath : path.resolve(baseDir2, rawPath);
        ...
        const db2 = drizzleSqlite(sqlite, { schema: sqlite_exports });
...
var nodeAssetsFetch = async (request, options) => {
  const { frontendDistPath: frontendDistPath2 } = options;
  const url = new URL(request.url);
  if (url.pathname.startsWith("/api/")) {
    return new Response("Not Found", { status: 404 });
  }
  let filePath = path2.resolve(frontendDistPath2, url.pathname.slice(1));
  if (!filePath.startsWith(frontendDistPath2)) {
  ...
  if (isRoot) {
    const indexFile = path2.join(frontendDistPath2, "index.html");
    try {
      if (fs2.statSync(indexFile).isFile()) {
        filePath = indexFile;
        isIndexHtml = true;
      }
    } catch (e) {
  ...
  if (!isIndexHtml) {
    ...
    try {
      initialStats = fs2.statSync(filePath);
    } catch (e) {
    }
    if (!initialStats || initialStats.isDirectory()) {
      ...
      if (!isStaticAsset || prefersHtml) {
        const fallbackPath = path2.join(frontendDistPath2, "index.html");
        try {
          if (fs2.statSync(fallbackPath).isFile()) {
  ...
  try {
    ...
    const content = fs2.readFileSync(fd);
    ...
    logger.info(`[Static] ${type} ${url.pathname} -> ${path2.basename(filePath)} (${mimeTypes[ext] || "bin"})`);
...
var frontendDistPath = path3.resolve(baseDir, "frontend/dist");
var dataDir = path3.resolve(baseDir, "data");
...
try {
  fs3.accessSync(dataDir, fs3.constants.W_OK);
} catch (err) {
  logger.error(`
...
var { db, executor } = await DbFactory.create();
var startupComplete = false;
var retries = 30;
var schemaFile = fs3.existsSync(path3.join(baseDir, "schema.sql")) ? path3.join(baseDir, "schema.sql") : path3.join(baseDir, "backend/schema.sql");
var isShareSchemaStatement = (sql3) => sql3.includes("CREATE TABLE IF NOT EXISTS share_links") || sql3.includes("CREATE TABLE IF NOT EXISTS share_audit_events") || sql3.includes("CREATE TABLE IF NOT EXISTS share_rate_limits") || sql3.includes("CREATE INDEX IF NOT EXISTS idx_share_links_") || sql3.includes("CREATE INDEX IF NOT EXISTS idx_share_audit_") || sql3.includes("CREATE INDEX IF NOT EXISTS idx_share_rate_limits_");
while (!startupComplete && retries > 0) {
  try {
    ...
    if (fs3.existsSync(schemaFile)) {
      const rawSchemaSql = fs3.readFileSync(schemaFile, "utf-8");
      ...
      for (const rawSql of statements) {
        ...
        const sql3 = transformSqlForDialect(rawSql, executor.engine);
        try {
          await executor.exec(sql3);
        } catch (e) {
...
cron.schedule("0 2 * * *", async () => {
  try {
    ...
    await handleScheduledBackup(envTemplate);
  } catch (e) {
...
serve({
  fetch: async (req) => {
    ...
    const env = {
      ...envTemplate,
      ASSETS: { fetch: (r) => nodeAssetsFetch(r, { frontendDistPath }) }
    };
    return app_default.fetch(req, env, {
      waitUntil: (p) => p.catch((e) => logger.error("[Hono] waitUntil failed:", e))
    });
  },
  port
}, (info) => {
