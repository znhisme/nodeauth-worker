const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-936a7cdd.js","assets/pdf-utils-r4RjNe6V.js","assets/compression-utils-CXh1ITwj.js","assets/vue-core-Daban9YF.js","assets/element-plus-Dh0klhaa.js","assets/element-plus-Dh61In7b.css","assets/simplewebauthn-3qpiAaRi.js","assets/tanstack-query-C-OQsQoR.js","assets/index-CLSE-HWx.css"])))=>i.map(i=>d[i]);
...
import { _ as hn, u as Tt, C as Yt, E as hl, G as tn, b as yl, H as _l, a as yn, i as wl, l as St, D as _n, f as wn, r as qo, __tla as __tla_0 } from "./index-936a7cdd.js";
    ...
    function yi(e, t, o, l = null) {
        ...
        const ue = async (f, y)=>{
            ...
            const { useAppLockStore: y } = await on(async ()=>{
                const { useAppLockStore: p } = await import("./index-936a7cdd.js").then(async (m)=>{
                    await m.__tla;
                    return m;
                }).then((C)=>C.Q);
                return {
                ...
                const { unmaskSecretFront: p } = await import("./index-936a7cdd.js").then(async (m)=>{
                    await m.__tla;
                    return m;
                }).then((C)=>C.P);
                return {
...
访问码：${p || "请在分享详情中查看"}`;
            ...
            if (!f) return "未记录";
            const y = Number(f);
            return Number.isFinite(y) && y > 0 ? new Date(y).toLocaleString() : "未记录";
        }, tt = (f)=>f === "active" ? "可访问" : f === "expired" ? "已过期" : f === "revoked" ? "已撤销" : "未知", kt = async ()=>{
            const f = await qo("/api/share"), y = f?.shares || f?.data?.shares || f?.data || [];
            return Array.isArray(y) ? y : [];
        }, Ue = (f, y)=>y.filter((G)=>G?.item?.id === f?.id), st = (f)=>{
            const y = f.map((G)=>`
                <div style="border:1px solid var(--el-border-color);border-radius:8px;padding:12px;margin:0 0 10px 0;">
                    <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;">
                        <strong>${Be(G.item?.service || "未命名服务")}</strong>
                        <span style="color:${G.status === "active" ? "var(--el-color-success)" : G.status === "revoked" ? "var(--el-color-danger)" : "var(--el-color-info)"};">${tt(G.status)}</span>
                    </div>
                    <div style="font-size:13px;color:var(--el-text-color-secondary);margin-top:6px;">账号：${Be(G.item?.account || "")}</div>
                    <div style="font-size:13px;color:var(--el-text-color-secondary);margin-top:6px;">创建：${Be(Ne(G.createdAt))}</div>
                    <div style="font-size:13px;color:var(--el-text-color-secondary);margin-top:6px;">过期：${Be(Ne(G.expiresAt))}</div>
                    <div style="font-size:13px;color:var(--el-text-color-secondary);margin-top:6px;">最近访问：${Be(Ne(G.lastAccessedAt))}</div>
                    <div style="font-size:13px;color:var(--el-text-color-secondary);margin-top:6px;">成功访问次数：${Number(G.accessCount || 0)}</div>
                </div>`).join("");
            return `<div style="text-align:left;max-height:360px;overflow:auto;">
                <p style="margin:0 0 12px 0;color:var(--el-text-color-secondary);">这里只显示安全元数据，不显示访问码、令牌、密码或 OTP 种子。</p>
                ${y}
                <p style="margin:12px 0 0 0;color:var(--el-color-warning);font-size:13px;">撤销只能阻止未来访问，不能收回收件人已经查看或复制的凭据。</p>
            </div>`;
        }, Ye = async (f)=>{
            try {
                if (!f?.id) return;
                ...
                const y = Ue(f, await kt());
                if (!y.length) {
                    await Jt.alert("此账户还没有分享链接。创建分享后可在这里查看状态、最近访问时间、访问次数并撤销链接。", "分享链接管理", {
                        confirmButtonText: "关闭"
                    });
                    return;
                }
                const G = y.find((p)=>p.status === "active");
                if (!G) {
                    await Jt.alert(st(y), "分享链接管理", {
                        dangerouslyUseHTMLString: !0,
                        confirmButtonText: "关闭"
                    });
                    return;
                }
                await Jt.confirm(st(y), "分享链接管理", {
                    dangerouslyUseHTMLString: !0,
                    type: "warning",
                    confirmButtonText: "撤销最新活跃链接",
                    cancelButtonText: "关闭",
                    distinguishCancelAndClose: !0
                });
                const ye = await qo(`/api/share/${encodeURIComponent(G.id)}`, {
                    method: "DELETE"
                });
                De.success(ye?.message || "分享链接已撤销");
                const p = Ue(f, await kt());
                p.length && await Jt.alert(st(p), "分享链接管理", {
                    dangerouslyUseHTMLString: !0,
                    confirmButtonText: "关闭"
                });
            } catch (y) {
                y !== "cancel" && y !== "close" && (St.error(y), De.error("加载分享管理失败"));
            }
        }, Ce = (f, y)=>{
            f === "edit" ? H(y) : f === "qr" ? m(y) : f === "delete" ? ie(y) : f === "restore" ? fe(y) : f === "share" ? de(y) : f === "shareManage" && Ye(y);
        }, fe = async (f)=>{
            if (!navigator.onLine || d.isOffline) {
