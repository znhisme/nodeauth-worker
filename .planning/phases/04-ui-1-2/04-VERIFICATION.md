---
phase: 04-ui-1-2
verified: 2026-05-03T22:19:24Z
status: gaps_found
score: 8/9 must-haves verified
overrides_applied: 0
gaps:
  - truth: "Backend source guarantees latest-share-wins for an owner and vault item."
    status: partial
    reason: "Sequential create/regenerate revokes older active shares, but revoke-plus-insert is not atomic; concurrent creates can leave multiple active links, matching WR-02 in 04-REVIEW.md."
    artifacts:
      - path: "src/features/share/shareService.ts"
        issue: "createShare calls revokeActiveForOwnerVaultItem and createShareLink as separate operations without a transaction or database-enforced active-share guard."
      - path: "src/shared/db/repositories/shareRepository.ts"
        issue: "Repository provides separate select/update revocation helper and separate insert path; no atomic replacement primitive exists."
      - path: "backend/dist/worker/worker.js"
        issue: "Generated Worker bundle contains the same non-atomic replacement sequence."
      - path: "backend/dist/docker/server.js"
        issue: "Generated Docker bundle contains the same non-atomic replacement sequence."
      - path: "backend/dist/netlify/api.mjs"
        issue: "Generated Netlify bundle contains the same non-atomic replacement sequence."
    missing:
      - "Add an atomic replace-active-share persistence primitive or a database-enforced active-share guard with retry/conflict handling across supported runtimes."
      - "Add a regression test that proves concurrent creates cannot leave multiple active owner+vault shares."
---
...
|---|-------|--------|----------|
| 1 | Backend source guarantees latest-share-wins for an owner and vault item. | PARTIAL | Sequential path exists: `ShareService.createShare()` revokes old shares before `createShareLink()` at `src/features/share/shareService.ts:89` and tests cover old-link inaccessibility. WR-02 is valid: the revoke and insert are separate operations, so concurrent creates can still leave multiple active links. |
| 2 | Latest-share-wins is enforced in backend service/repository logic, not simulated by frontend UI. | VERIFIED | `ShareRepository.revokeActiveForOwnerVaultItem()` exists at `src/shared/db/repositories/shareRepository.ts:62`; `ShareService.createShare()` calls it at `src/features/share/shareService.ts:89`; frontend implementation is gated, not patched. |
...
| 7 | Worker, Docker, and Netlify backend bundles are regenerated and contain Phase 4 behavior. | VERIFIED | Bundle marker checks found `revokeActiveForOwnerVaultItem`, `createSharesForOwnerBatch`, `share_inaccessible`, `Cache-Control`, and `Referrer-Policy` in all three backend bundles. |
...
|----------|----------|--------|---------|
| `src/shared/db/repositories/shareRepository.ts` | Owner+vault active-share revocation helper | VERIFIED | Exists and exports `revokeActiveForOwnerVaultItem`; uses owner, vault item, `revokedAt IS NULL`, and `expiresAt > revokedAt` predicates. Atomic replacement is missing. |
...
| `src/features/share/shareService.ts` | Latest-share-wins and batch creation semantics | PARTIAL | Sequential replacement and batch delegation are implemented; latest-share-wins is not concurrency-safe. |
...
|------|----|-----|--------|---------|
| `ShareService.createShare` | `ShareRepository.revokeActiveForOwnerVaultItem` | Direct repository call after owner validation | PARTIAL | Link is wired at `src/features/share/shareService.ts:89`, but replacement is not atomic with the later insert. |
...
| `shareService.ts` create path | `replacedShares` | `shareRepository.revokeActiveForOwnerVaultItem(...)` | Yes, sequentially | PARTIAL - data flows, but not atomic with insert |
...
|-------------|-------------|--------------------|--------|----------|
| PH4-BE-01 | 04-01 | ROADMAP/VALIDATION/PLAN, not REQUIREMENTS.md | PARTIAL | Sequential latest-share-wins and old-link inaccessibility are implemented/tested, but WR-02 shows the guarantee fails under concurrent creates. |
...
|------|------|---------|----------|--------|
| `src/features/share/shareService.ts` | 89 | Non-atomic revoke then insert | Blocker | Concurrent creates can violate latest-share-wins by leaving multiple active shares. |
...
Phase 4 mostly achieved the planned backend and source-gate work: batch sharing is wired and tested, runtime bundles contain the new backend behavior, and the unavailable frontend source is handled by an explicit gate instead of generated asset patching.

The phase goal overstates latest-share-wins as a backend guarantee. The implementation enforces replacement sequentially, but it is not atomic at the persistence boundary. WR-02 is therefore a real phase gap and should be closed before declaring Phase 4 passed.

---

_Verified: 2026-05-03T22:19:24Z_
