...
1. **Public route token placement**
   ...
   - RESOLVED: Phase 2 implements API-only recipient access under `/api/share/public/:token/access` and keeps `/share/:token` as the future UI URL generated by `buildShareUrl()`. This matches the approved UI-SPEC API-only scope and avoids editing generated frontend assets. [VERIFIED: `02-UI-SPEC.md`, `buildShareUrl()`]
...
2. **Password field semantics in `SharedItemView`**
   ...
   - RESOLVED: Phase 2 maps only fields available in the restored backend source and schema, supports current OTP code/countdown when OTP data exists, and does not invent a separate password/login-secret schema or frontend model. [VERIFIED: `backend/schema.sql`, `02-UI-SPEC.md`]
...
3. **Logger redaction implementation**
   ...
   - RESOLVED: Phase 2 includes logger redaction because REC-06 requires public endpoint responses and logs to avoid leaking sensitive values; Phase 3 hardening can add broader regression coverage but must not be the first implementation of share-token log protection. [VERIFIED: REC-06, `02-UI-SPEC.md`]
